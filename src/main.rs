...
fn switch(routes: &Route) -> Html {
    match routes {
        Route::Home => html! {
            <div class="m-6">
                <p class="title is-3">{ "Welcome to Updoot!" }</p>
                <p>
                    { "Updoot is a simple game written with Rust 🦞. To read the code, check out the repository on " }
                    <a href="https://github.com/evklein/updoot">{"Github."}</a>
                </p>
                <br />
                <p class="title is-4">{ "How to Play" }</p>
                <p>
                    { "Updoot grabs recent comments from "}
                    <a href="https://news.ycombinator.com/">{" HackerNews "}</a>
                    {". The player is shown a parent comment, and then 2 child comments." }
                </p>
                <p>
                    { "One of the child comments is real, one of them is generated by " }
                    <a href="https://beta.openai.com/docs/">{ "OpenAI's GPT-3 autoregressive language model API" }</a>
                    {". It's your job as the player to beat the machine and choose the fake response."}
                </p>
                <br />
                <b>{ "GL;HF!" }</b>
            </div>
